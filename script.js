...
/* ══════ CONVERGENCIA — cálculo compartido ══════ */
function calcConvergence(data) {
  const mun = state.level === 'municipal' ? data.mun : data.dep;
  const dir = getIndicatorDir(data);
  const pairs = [];
  for (const [, entry] of mun) {
  ...
  if (r < -0.3)      { colorClass='conv-converge'; lineColor='#16a34a'; label='Convergencia'; icon='↘↗'; }
  else if (r > 0.3)  { colorClass='conv-diverge';  lineColor='#2563eb'; label='Divergencia';  icon='↗↘'; }
  else               { colorClass='conv-neutral';   lineColor='#9ca3af'; label='Sin patrón';   icon='→';  }
  return { pairs, n, r, rAbs, r2, tStat, pSig, slope, intercept, mx, my, colorClass, lineColor, label, icon };
}

/* ══════ CONVERGENCIA TERRITORIAL — sidebar mínimo ══════ */
function renderConvergence(data) {
  /* Quitamos la sección del sidebar si existía */
  const old = $('convergence-section');
  if (old) old.remove();
}

/* ══════ CONVERGENCIA — vista completa ══════ */
function renderConvergenciaView(data) {
  ...
  if (indName) indName.textContent = ind ? ind.label : '—';

  const cv = calcConvergence(data);
  if (!cv) {
    if (badge)     badge.innerHTML = '';
    if (stats)     stats.innerHTML = '';
    if (container) container.innerHTML = '<div class="conv-scatter-placeholder"><p>Datos insuficientes para calcular convergencia</p></div>';
    return;
  }

  const { pairs, n, r, r2, tStat, pSig, slope, intercept, colorClass, lineColor, label, icon } = cv;
  const rAbs = Math.abs(r);

  /* Badge resultado */
  if (badge) badge.innerHTML = `<div class="conv-badge-big ${colorClass}"><span class="conv-badge-icon">${icon}</span><span class="conv-badge-label">${label}</span><span class="conv-badge-r">r = ${r>=0?'+':''}${r.toFixed(3)}</span></div>`;
  ...
  if (stats) stats.innerHTML = `
    ...
    <div class="conv-stat-row"><span class="conv-stat-k">t-stat</span><span class="conv-stat-v">${tStat.toFixed(2)}</span></div>
    <div class="conv-stat-row"><span class="conv-stat-k">Sig.</span><span class="conv-stat-v">${sigHTML}</span></div>
    <div class="conv-stat-row"><span class="conv-stat-k">n munic.</span><span class="conv-stat-v">${n}</span></div>`;

  /* ── Scatter grande ── */
  if (!container) return;
  const W = 640, H = 420;
  const ML = 58, MR = 20, MT = 20, MB = 48;
  ...
  const yStep  = yRange > 200 ? 50 : yRange > 50 ? 10 : yRange > 10 ? 5 : yRange > 2 ? 1 : yRange > 0.5 ? 0.2 : 0.05;
  const yTickStart = Math.ceil((yMin+yPad*0.5)/yStep)*yStep;
  const yTicks = [];
  for (let v=yTickStart; v<=yMax-yPad*0.5; v+=yStep) yTicks.push(Math.round(v*10000)/10000);

  const yGridSVG = yTicks.map(v => {
    const y = scY(v).toFixed(1);
    return `<line x1="${ML}" y1="${y}" x2="${ML+PW}" y2="${y}" stroke="#e5e7eb" stroke-width="${v===0?1:0.6}" stroke-dasharray="${v===0?'none':'3,3'}"/>
  ...
  const xTicks = [];
  for (let v=xTickStart; v<=xMax-xPad*0.5; v+=xStep) xTicks.push(Math.round(v*10000)/10000);
  ...
  const dotsSVG = pairs.map(p => {
    ...
    return `<circle cx="${cx}" cy="${cy}" r="4.5" fill="${fill}" fill-opacity="0.7" stroke="white" stroke-width="0.8"><title>${p.name}&#10;Valor inicial: ${fmt(p.v1)}&#10;Cambio favorable: ${fmt(p.favorable)}</title></circle>`;
  }).join('');

  const yCtr = (MT+PH/2).toFixed(1);

  const svg = `<svg width="100%" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    <defs><clipPath id="cv-clip"><rect x="${ML}" y="${MT}" width="${PW}" height="${PH}"/></clipPath></defs>
    ...
    <text x="${(ML+PW/2).toFixed(1)}" y="${(H-6).toFixed(1)}" text-anchor="middle" font-size="13" fill="#6b7280" font-weight="600">Valor inicial</text>
    <text x="14" y="${yCtr}" text-anchor="middle" dominant-baseline="middle" font-size="13" fill="#6b7280" font-weight="600" transform="rotate(-90 14 ${yCtr})">Cambio favorable</text>
  </svg>`;
