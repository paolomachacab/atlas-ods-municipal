...
const ALL_THRESHOLDS = {
  ...
  '03_12_01_14_ppv': { meta:{max:100,min:83.35}, proximo:{max:83.35,min:61.07}, retos:{max:61.07,min:38.80}, grandes:{max:38.80,min:0} },
  ...
  '09_08_01_01_ci':  { meta:{max:100,min:71.85}, proximo:{max:71.85,min:58.13}, retos:{max:58.13,min:44.42}, grandes:{max:44.42,min:0} },
  ...
  '16_07_02_03_tcs': { meta:{max:0,min:3}, proximo:{max:3,min:6}, retos:{max:6,min:10}, grandes:{max:10,min:100} },
  '16_09_01_04_nic': { meta:{max:100,min:95}, proximo:{max:95,min:90}, retos:{max:90,min:85}, grandes:{max:85,min:0} },
  '17_18_01_02_ipc': { meta:{max:3753.43,min:463.09}, proximo:{max:463.09,min:252.88}, retos:{max:252.88,min:42.67}, grandes:{max:42.67,min:0} },
};
...
const INDICATOR_FILES = [
  ...
  { file: '01_04_01_04_psv.xlsx', label: 'Personas sin vivienda adecuada',                               dir: 'desc' },
  { file: '02_02_02_01_smu.xlsx', label: 'Sobrepeso en mujeres (15-49 años)',                             dir: 'desc' },
  ...
  { file: '04_03_04_04_pes.xlsx', label: 'Población con educación superior (mayores de 19 años)',         dir: 'asc'  },
  { file: '04_06_01_06_alf.xlsx', label: 'Tasa de alfabetización (mayores de 15 años)',                   dir: 'asc'  },
  ...
  { file: '15_05_02_02_pbd.xlsx', label: 'Pérdida de biodiversidad por deforestación',                   dir: 'desc' },
  { file: '15_05_02_03_pbi.xlsx', label: 'Pérdida de biodiversidad por incendios y minería',             dir: 'desc' },
  ...
  { file: '16_07_02_03_tcs.xlsx', label: 'Bloqueos y manifestaciones',                                   dir: 'desc' },
...
for (const ind of INDICATOR_FILES) INDICATOR_DIR_MAP[ind.file.replace('.xlsx', '')] = ind.dir;
...
async function loadAvailableIndicators() {
  ...
  for (const ods of Object.keys(groups).sort()) {
    ...
    for (const ind of groups[ods]) {
      ...
      opt.value = ind.file;
      opt.textContent = (ind.dir === 'asc' ? '↑ ' : '↓ ') + ind.label;
      grp.appendChild(opt);
    }
...
function renderStats(data) {
  ...
  // Conteo por clasificación en período final
  const cats = { meta:0, proximo:0, retos:0, grandes:0, sin:0 };
  for (const [, entry] of mun) cats[classify(entry.v2 ?? entry.v1, data.thresholds)]++;
  ...
  dom.statsContainer.innerHTML = `
    ...
    <div class="stats-period-row">
      <div class="stats-period-col stats-period-initial">
        <div class="stats-period-label">${data.colLabels.v1}</div>
      ...
      <div class="stats-period-col stats-period-final">
        <div class="stats-period-label">${data.colLabels.v2}</div>
    ...
    <div class="stats-cats-label">Clasificación municipios · ${data.colLabels.v2}</div>
    <div class="stats-cats-grid">
      <div class="stats-cat-chip status-meta">${cats.meta} <span>Meta</span></div>
      <div class="stats-cat-chip status-proximo">${cats.proximo} <span>Próx.</span></div>
      <div class="stats-cat-chip status-retos">${cats.retos} <span>Retos</span></div>
      <div class="stats-cat-chip status-grandes">${cats.grandes} <span>Grandes</span></div>
    </div>
  `;
...
function renderConvergence(data) {
  ...
  if (r < -0.3)      { icon='↘↗'; label='Convergencia';   colorClass='conv-converge'; detail='Los municipios más rezagados mejoran proporcionalmente más.'; }
  else if (r > 0.3)  { icon='↗↘'; label='Divergencia';    colorClass='conv-diverge';  detail='Los municipios con mejor posición inicial avanzan más rápido.'; }
  else               { icon='→';  label='Sin patrón claro'; colorClass='conv-neutral'; detail='No hay relación consistente entre posición inicial y ritmo de mejora.'; }
  ...
  convSection.innerHTML = `
    ...
    <div class="conv-card ${colorClass}">
      <div class="conv-header">
        ...
        <span class="conv-r">r = ${r >= 0 ? '+' : ''}${r.toFixed(2)}</span>
      </div>
      ...
      <div class="conv-meta">Basado en ${n} municipios con datos en ambos períodos</div>
    </div>`;
}
...
async function onIndicatorChange(file) {
  ...
  tableMode = 'indicator'; // reset al cambiar indicador
