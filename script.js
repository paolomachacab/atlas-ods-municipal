/**
 * ═══════════════════════════════════════════════════════════════
 *  ATLAS TERRITORIAL BOLIVIA · script.js  v8
 * ═══════════════════════════════════════════════════════════════
 */
'use strict';
...
/* ══════ COLORES ODS ACTUALIZADOS ══════ */
const ODS_COLORS = {
  meta:    '#29b158',   // verde
  proximo: '#ebc849',   // amarillo
  retos:   '#ec8444',   // naranja
  grandes: '#e04350',   // rojo
  sin:     '#d1d5db',   // gris sin datos
};

/* ══════ DETECCIÓN DE CAMPOS — incluye Codigo_INE ══════ */
const FIELD_CANDIDATES = {
  id: [
    'Codigo_INE', 'codigo_ine', 'CODIGO_INE',
    'COD', 'cod', 'COD_MUN', 'cod_mun', 'CODMUN', 'codmun',
    'INE', 'ine', 'CODIGO', 'codigo',
    'ID', 'id', 'OBJECTID', 'GID',
  ],
  name: [
    'Municipio', 'MUNICIPIO', 'municipio',
    'NOM_MUN', 'nom_mun', 'NOMBRE', 'nombre',
    'DEP_MUN_TI', 'NAME', 'name', 'NOM', 'nom',
  ],
};
...
function showTooltip(mouseEvent, feature) {
  ...
  const dpto  = props.Depart || props.DEPARTAMEN || '';
...
function onFeatureClick(e, feature) {
  ...
  dom.munInfoCard.innerHTML = `
    ...
    ${dpto ? `<div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">${dpto}${prov ? ' · ' + prov : ''}</div>` : ''}
    <div class="mun-card-row"><span class="mun-card-key">Código INE</span><span class="mun-card-val">${props.Codigo_INE || props.COD || id}</span></div>
