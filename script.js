...
function renderConvergence(data) {
  ...
  let icon, label, detail, colorClass, lineColor;
  if (r < -0.3) {
    icon = '↘↗'; label = 'Convergencia territorial'; colorClass = 'conv-converge'; lineColor = '#16a34a';
    detail = 'Los municipios con más dificultades mejoran más rápido que los que ya estaban bien. La brecha se reduce.';
  } else if (r > 0.3) {
    icon = '↗↘'; label = 'Divergencia territorial'; colorClass = 'conv-diverge'; lineColor = '#2563eb';
    detail = 'Los municipios que ya estaban mejor siguen mejorando más rápido. La brecha entre municipios crece.';
  } else {
    icon = '→'; label = 'Sin patrón definido'; colorClass = 'conv-neutral'; lineColor = '#9ca3af';
    detail = 'No se detecta una tendencia clara: algunos rezagados mejoran más, otros menos.';
  }
  ...
  /* clamp regression line endpoints to plot y-range */
  function lineEndpoint(xVal) {
    const yVal = intercept + slope * xVal;
    return { x: scX(xVal), y: scY(Math.max(yMin, Math.min(yMax, yVal))) };
  }
  const lp1 = lineEndpoint(xMin + xPad * 0.1);
  const lp2 = lineEndpoint(xMax - xPad * 0.1);
  ...
  /* Y-axis: 3 ticks — min, 0 (if in range), max */
  const rawYTicks = [Math.min(...ys), Math.max(...ys)];
  if (yMin < -yPad*0.5 && yMax > yPad*0.5) rawYTicks.push(0);
  const yTicksSVG = rawYTicks.map(v => {
    const y = scY(v).toFixed(1);
    const isDashed = v === 0 ? '' : 'stroke-dasharray="2,2"';
    return `<line x1="${ML}" y1="${y}" x2="${ML+PW}" y2="${y}" stroke="#e2e5ea" stroke-width="0.5" ${isDashed}/>
            <text x="${(ML-3).toFixed(1)}" y="${y}" text-anchor="end" dominant-baseline="middle" font-size="7.5" fill="#9ca3af">${fmtTick(v)}</text>`;
  }).join('');

  /* X-axis: 2 ticks — min and max raw values */
  const xTicksSVG = [Math.min(...xs), Math.max(...xs)].map(v => {
    const x = scX(v).toFixed(1);
    return `<text x="${x}" y="${(MT+PH+14).toFixed(1)}" text-anchor="middle" font-size="7.5" fill="#9ca3af">${fmtTick(v)}</text>`;
  }).join('');

  /* Zero line on Y if applicable */
  const zeroY = (yMin < 0 && yMax > 0) ? scY(0).toFixed(1) : null;
  const zeroLine = zeroY ? `<line x1="${ML}" y1="${zeroY}" x2="${ML+PW}" y2="${zeroY}" stroke="#9ca3af" stroke-width="0.8"/>` : '';
  ...
  const dotsSVG = pairs.map(p => {
    ...
    const tipText = `${p.name}: x=${fmtTick(p.v1)}, y=${fmtTick(p.favorable)}`;
    return `<circle cx="${cx}" cy="${cy}" r="2.5" fill="${fill}" fill-opacity="0.75" stroke="white" stroke-width="0.5"><title>${tipText}</title></circle>`;
  }).join('');

  const yCenterLabel = (MT + PH / 2).toFixed(1);

  const scatterSVG = `
  <svg width="100%" viewBox="0 0 ${VW} ${VH}" xmlns="http://www.w3.org/2000/svg" class="conv-scatter" style="display:block;margin:8px 0 4px">
    <defs>
      <clipPath id="conv-clip">
        <rect x="${ML}" y="${MT}" width="${PW}" height="${PH}"/>
      </clipPath>
    </defs>
    <rect x="${ML}" y="${MT}" width="${PW}" height="${PH}" fill="#f9fafb" rx="2"/>
    ${yTicksSVG}
    ${zeroLine}
    <line x1="${ML}" y1="${(MT+PH).toFixed(1)}" x2="${(ML+PW).toFixed(1)}" y2="${(MT+PH).toFixed(1)}" stroke="#d1d5db" stroke-width="0.8"/>
    <line x1="${ML}" y1="${MT}" x2="${ML}" y2="${(MT+PH).toFixed(1)}" stroke="#d1d5db" stroke-width="0.8"/>
    ${xTicksSVG}
    <text x="${(ML + PW/2).toFixed(1)}" y="${(VH-1).toFixed(1)}" text-anchor="middle" font-size="8.5" fill="#6b7280" font-weight="600">Valor inicial</text>
    <text x="7" y="${yCenterLabel}" text-anchor="middle" dominant-baseline="middle" font-size="8.5" fill="#6b7280" font-weight="600" transform="rotate(-90 7 ${yCenterLabel})">Cambio favorable</text>
    <g clip-path="url(#conv-clip)">
      ${dotsSVG}
      <line x1="${lp1.x.toFixed(1)}" y1="${lp1.y.toFixed(1)}" x2="${lp2.x.toFixed(1)}" y2="${lp2.y.toFixed(1)}" stroke="${lineColor}" stroke-width="1.8" stroke-linecap="round" opacity="0.9"/>
    </g>
  </svg>`;
  ...
  convSection.innerHTML = `
    ...
    <div class="conv-card ${colorClass}">
      ...
      <div class="conv-stats-inline">
        <span>R² = ${r2.toFixed(3)}</span>
        <span class="conv-stats-dot">·</span>
        ${sigLabel}
        <span class="conv-stats-dot">·</span>
        <span>n = ${n}</span>
      </div>
      ...
      <div class="conv-bar-wrap">
        ...
        <div class="conv-bar-labels"><span>← Brecha se reduce</span><span>Brecha crece →</span></div>
      </div>
      <div class="conv-meta">Correlación de Pearson · n = ${n} municipios</div>
    </div>`;
}
