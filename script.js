...
function showView(view) {
  ...
  // Panel convergencia ocupa todo el espacio
...
function classify(value, thresholds, forceDir) {
  ...
  // Indicadores de paridad/ratio: meta tiene un rango acotado [min, max] alrededor de 1
  // Valores mayores a meta.max son igual de problemáticos que valores menores a meta.min
  // Se detectan porque son ascendentes con meta.max definido (no igual a min) y max <= 2
  ...
  if (isBidirectional) {
    // Dentro del rango meta: OK
    if (value >= t.meta.min - eps && value <= t.meta.max + eps) return 'meta';
    // Fuera del rango: reflejar el valor excedente alrededor del centro del rango meta
    const center = (t.meta.max + t.meta.min) / 2;
    const dist = Math.abs(value - center);
    const halfMeta = (t.meta.max - t.meta.min) / 2;
    // Usar la distancia al centro para clasificar simétricamente
    const adjValue = center - (dist - halfMeta); // valor equivalente en el lado bajo
  ...
  if (ascending) {
    // Para valores que superan el límite max definido → meta
    // (umbrales con límite fijo que los datos reales superan, ej: cap=100%)
...
function renderConvergenciaView(data) {
  ...
  // Usar tamaño real del contenedor — gráfico rectangular (ancho > alto)
  ...
  // Limitar altura para que sea rectangular (máx 60% del ancho)
  const W = availW;
