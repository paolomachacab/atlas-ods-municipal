...
function calcConvergence(data) {
  ...
  const pairs = [];
  for (const [, entry] of mun) {
    if (entry.v1 !== null && entry.v2 !== null && Math.abs(entry.v1) > 1e-10) {
      const rawChange = dir === 'asc' ? (entry.v2 - entry.v1) : -(entry.v2 - entry.v1);
      const growthRate = rawChange / Math.abs(entry.v1);   // β-conv: cambio / valor inicial
      pairs.push({
        v1: entry.v1,
        favorable: growthRate,
        name: entry.name || '',
...
function renderConvergenciaView(data) {
  ...
  if (badge) badge.innerHTML = `
    <div class="conv-badge-big ${colorClass}">
      <span class="conv-badge-icon">${icon}</span>
      <span class="conv-badge-label">${label}</span>
      <span class="conv-badge-r">β = ${slope>=0?'+':''}${slope.toFixed(4)}</span>
      <span class="conv-badge-r2">r = ${r>=0?'+':''}${r.toFixed(3)}</span>
    </div>`;
  ...
  const W = 640, H = 420;
  const ML = 62, MR = 20, MT = 20, MB = 52;
  const PW = W-ML-MR, PH = H-MT-MB;

  const colLabels = data.colLabels || {};
  const xAxisLabel = `${indLabel} — ${colLabels.v1 || 'Período inicial'}`;
  const yAxisLabel = `Cambio favorable / Valor inicial`;
  ...
  const dotsSVG = pairs.map((p,i) => {
    ...
    return `<circle class="cv-dot" cx="${cx}" cy="${cy}" r="4.5" fill="${fill}" fill-opacity="0.72" stroke="white" stroke-width="0.8" data-name="${p.name.replace(/"/g,'&quot;')}" data-x="${fmt(p.v1)}" data-y="${fmt(p.favorable)}" style="cursor:pointer"/>`;
  }).join('');

  const yCtr = (MT+PH/2).toFixed(1);

  const svg = `<svg id="cv-svg" width="100%" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="overflow:visible">
  ...
  /* Tooltip hover JS */
  const tooltip = $('conv-dot-tooltip');
  container.querySelectorAll('.cv-dot').forEach(dot => {
    dot.addEventListener('mouseenter', e => {
      const name = dot.dataset.name;
      const xv   = dot.dataset.x;
      const yv   = dot.dataset.y;
      tooltip.innerHTML = `<strong>${name}</strong><br>Valor inicial: ${xv}<br>Cambio / inicial: ${yv}`;
