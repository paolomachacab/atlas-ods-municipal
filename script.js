...
function renderConvergenciaView(data) {
  ...
  const yAxisLabel = convYMode === 'relative' ? 'Cambio relativo (Δ / valor inicial)' : 'Cambio absoluto (período final − inicial)';
  ...
  container.querySelectorAll('.cv-dot').forEach(dot => {
    dot.addEventListener('mouseenter', e => {
      ...
      const yLabel = convYMode === 'relative' ? 'Cambio relativo' : 'Cambio absoluto';
      tooltip.innerHTML = `<strong>${name}</strong><br>Valor inicial: ${xv}<br>${yLabel}: ${yv}`;
    ...
    dot.addEventListener('click', e => {
      ...
      if (munId) {
        showView('maps');
        setTimeout(() => zoomToMunicipality(munId), 150);
      }
    });
  });
}
